...
books_df = pd.read_csv(r"./book_data.csv")
...
books_df["processed_description"] = books_df["book_desc"].apply(preprocess_text)
...
title_to_index = pd.Series(
    books_df.index, index=books_df["book_title"].apply(lambda x: x.lower())
)


# Recommendation function
def recommend_books(
    title, model=model, books_df=books_df, title_to_index=title_to_index
):
    ...
    data = {
        "titles": books_df["book_title"].iloc[indices].values.tolist(),
        "urls": books_df["image_url"].iloc[indices].values.tolist(),
    }
    return data
